"""Tier 1 sub-category drill-downs (YouTube, 2026) — no new classification.

Sister to plot_drilldown_tier1.py (TikTok). Uses only fields already on the
YouTube CSVs:

  1. positive_register_split_2026_youtube.png
     Each positive theme bar split by _register.

  2. drilldown_keywords_backlash_2026_youtube.png /
     drilldown_keywords_positive_2026_youtube.png
     Small-multiples: top seed search keywords per theme.
"""
...
def plot_register_split(p):
    ...
    ax.set_title("How organic is each positive theme?  YouTube positive content, 2026\n"
    ...
    fig.tight_layout()
    out = OUT / "positive_register_split_2026_youtube.png"
...
def main():
    ...
    print(f"Loaded YouTube 2026: backlash n={len(b):,}  positive n={len(p):,}")

    o1 = plot_register_split(p)
    print("wrote", o1.name)
    o2 = plot_keyword_smallmultiples(
        b, "_topic_primary", BACKLASH_LABELS,
        "AI backlash — top seed keywords per theme (proxy sub-categories)  ·  YouTube 2026",
        "drilldown_keywords_backlash_2026_youtube.png")
    print("wrote", o2.name)
    o3 = plot_keyword_smallmultiples(
        p, "_theme", POSITIVE_LABELS,
        "AI positive — top seed keywords per theme (proxy sub-categories)  ·  YouTube 2026",
        "drilldown_keywords_positive_2026_youtube.png")
    print("wrote", o3.name)
